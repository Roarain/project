...
import scrapy
import sys
from ..items import OpmpersistentItem
import re
import time
...
logging.basicConfig(filename='persistent.log', level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')

reload(sys)
sys.setdefaultencoding('utf-8')


class Persistent(scrapy.Spider):
    ...
    def parse(self, response):
		...
		key_hash = hmac.new(key, self.salt, hashlib.sha256).hexdigest() + 'P'
		d = dict(
